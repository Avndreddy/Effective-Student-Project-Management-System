...
const REP = () => {
  ...
  const getData = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        setAdditionalArgument(data.length)
  ...
  const handleProjectClick = (project) => {
    localStorage.setItem("myarraycomment",project.comments);
  ...
  // const comments1=selectedProject.comments;
  // console.log(comments1)
  // localStorage.setItem("myarraycomment",comments1);
  // console.log(selectedProject.comments)
  if (!isLoggedIn) {
          ...
          return (
            <Card key={index}>
              <CardContent>
                <Link
                  to={`/rep/${
                    selectedProject._id != undefined && selectedProject._id
                  }/${weekKey}}`}
                >
